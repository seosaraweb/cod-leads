    ...
    <>
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .filters-grid { flex-direction: column !important; }
        }
      `}</style>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16, flexWrap:'wrap', gap:10 }}>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          <button onClick={()=>setShowFilters(f=>!f)} style={{ padding:'9px 16px', background:'#fff', border:'1.5px solid #e0e0e0', borderRadius:9, cursor:'pointer', fontSize:14 }}>🔍 Filtres</button>
      ...
      {showFilters && (
        <div style={{ background:'#fff', borderRadius:14, padding:'16px', marginBottom:16, boxShadow:'0 1px 6px rgba(0,0,0,0.05)' }}>
          <div className="filters-grid" style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'flex-end' }}>
            ...
            <div><div style={{ fontSize:12, color:'#888', marginBottom:4 }}>Statut</div>
            ...
            {user?.role==='admin' && (
              <div><div style={{ fontSize:12, color:'#888', marginBottom:4 }}>Support</div>
            ...
            <table className="orders-table">
              <thead>
                <tr style={{ background:'#f9f9f9', borderBottom:'2px solid #f0f0f0' }}>
                  {['Réf','Produit','Client','Téléphone','Ville','Prix','Statut', user?.role==='admin'&&'Support','Heure',''].filter(Boolean).map(h=>(
                    <th key={h} style={{ padding:'12px 14px', textAlign:'left', fontWeight:600, color:'#666', fontSize:13, whiteSpace:'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {orders.map(o => (
                  <tr key={o.id} style={{ borderBottom:'1px solid #f5f5f5' }}
                    ...
                    <td style={{ padding:'11px 14px', fontFamily:'monospace', fontSize:12, fontWeight:700, color:'#2563eb', whiteSpace:'nowrap' }}>{o.order_ref}</td>
                    <td style={{ padding:'11px 14px', maxWidth:160 }}>
                      <div style={{ fontWeight:500, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', fontSize:14 }}>{o.product_name}{o.size?` (${o.size})`:''}</div>
                      {o.notes && <div style={{ fontSize:11, color:'#aaa' }}>{o.notes}</div>}
                    </td>
                    <td style={{ padding:'11px 14px', fontWeight:600, fontSize:14 }}>{o.client_name}</td>
                    <td style={{ padding:'11px 14px', fontFamily:'monospace', fontSize:13 }}>{o.phone}</td>
                    <td style={{ padding:'11px 14px', fontSize:14 }}>{o.city}</td>
                    ...
                    {user?.role==='admin' && <td style={{ padding:'11px 14px', color:'#888', fontSize:13 }}>{o.support_name}</td>}
                    ...
                    <td style={{ padding:'11px 14px' }}>
                      {user?.role==='admin' && <button onClick={()=>del(o.id)} style={{ background:'none', border:'none', cursor:'pointer', fontSize:16, color:'#ddd', padding:4 }}>🗑️</button>}
                    </td>
                  </tr>
                ))}
              </tbody>
                ...
                {expanded===o.id && (
                  <div style={{ borderTop:'1px solid #f0f0f0', padding:'12px 14px', background:'#fafafa' }}>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:10 }}>
                      {[['Produit',o.product_name+(o.size?` (${o.size})`:'')+( o.quantity>1?` ×${o.quantity}`:'')],['Support',o.support_name],['Adresse',o.address]].map(([k,v])=>(
